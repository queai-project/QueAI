...
- **CI in `.github/workflows/`:** `ci.yml` runs ruff lint + tests on Python 3.11/3.12. `sync-installer.yml` mirrors install.sh into the landing repo so `queai.dev/install.sh` stays in sync. No container registry — users build the image locally via the installer.
...
- **Phase 2 block D adds three modules**: `core/healthcheck.py` (probes `healthcheck_entry_point` via Traefik, locmem cache), `core/audit.py` (decorator + auto-purge based on `QUEAI_AUDIT_MAX_EVENTS`/`KEEP_AFTER_PURGE`), `core/backup.py` (light tar.gz with db.sqlite3 + .envs, not the plugins themselves). The audit decorator `@audit_record(action, source)` reads `manifest_folder_name` from POST or `folder_name` from kwargs for the target. The SSE logs endpoint limits to 2 simultaneous streams via `threading.BoundedSemaphore` — gunicorn sync workers would otherwise hang.
